...
public class JFrmProduto extends JFrame {
	...
	public JFrmProduto() { 
		...
		jBtnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (Diversos.confirmar("Deseja excluir", titulo))
					if (!verificaChaveV() && !verificaChaveL())
						cadastrarDados('E');
				else
					Diversos.mostrarDados
					("N�o pode remover\n O produto " + jTxtDesc.getText() +
					" pois est� sendo manipulado pela venda e ou no lote" , titulo, false);
			}
		});
		...
		jTxtCodigo.setBackground(Color.CYAN);
		...
		jTxtDesc.setBackground(Color.CYAN);
		...
		jLblPrecoU.setBounds(223, 115, 140, 21);
		jContentPane.add(jLblPrecoU);
		
		jTxtPrecoU = new JTextField();
		jTxtPrecoU.setBackground(Color.CYAN);
		...
		jTxtPrecoU.setBounds(389, 114, 81, 25);
		jContentPane.add(jTxtPrecoU);
		
		JPanel jPnlPerecivel = new JPanel();
		jPnlPerecivel.setBackground(Color.CYAN);
		jPnlPerecivel.setBorder(new TitledBorder(new LineBorder(new Color(0, 0, 0), 2), "Perec�vel",
				 TitledBorder.CENTER, TitledBorder.TOP,new Font("DejaVu Sans", Font.BOLD | Font.ITALIC, 13), null));
		jPnlPerecivel.setBounds(512, 155, 134, 65);
		jContentPane.add(jPnlPerecivel);
		...
		jPnlPerecivel.add(jChkSim);
		 ...
		 jPnlEstoque.setBorder(new TitledBorder(new LineBorder(new Color(0, 0, 0), 2), "Estoque", TitledBorder.CENTER, TitledBorder.TOP,
		 				new Font("DejaVu Sans", Font.BOLD | Font.ITALIC, 13), null));
		 jPnlEstoque.setBackground(Color.CYAN);
		 ...
		 jCmbGrupo.setBackground(Color.CYAN);
	...
	private void limpar() {
		 JTextField txt[] = {jTxtCodigo, jTxtDesc, jTxtPrecoU};
	     ...
	     JButton jBtn[] = {jBtnAlterar, jBtnExcluir, jBtnIncluir};
	        for (JButton btn : jBtn) 
	            btn.setEnabled(false);  //desabilita os bot�es  
	     jTxtDesc.requestFocusInWindow();
	}
	
	private void relatorioGeral() {
      //  jTxtARelatorio.setText(cP.relatorioGeral());
    }
	...
	private void carregaLista() {
		String grupo[] = {"Carnes", "Hortifruti", "Latic�nios", "Limpeza", "Massas","Peixaria"};
        for(String g : grupo)
        	jCmbGrupo.addItem(g);
        }     
	...
	private void carregaObjetos(Produto p) {
      ...
      jChkSim.setSelected(p.isPerecivel());   
	...
	private void cadastrarDados(char opcao) {
	         ...
	         if(jTxtCodigo.getText().isEmpty() || jTxtDesc.getText().isEmpty() || 
	              ...
	              resp = "Favor digitar os dados";
	                  ...
	                  p.setPerecivel(jChkSim.isSelected());
        ...
        if (!Diversos.testaNum(jTxtCodigo.getText(), titulo))
            jTxtCodigo.setText(""); // converter texto para numero
        		   ...
        		   (Integer.parseInt(jTxtCodigo.getText()), 0 ,0 , titulo))
                       jTxtCodigo.setText(""); //testar se � maior que zero
               else {
	...
	private boolean verificaChaveV() {
	    ...
	    String parametro[] = {jTxtCodigo.getText(), "codigo", "Venda"};
		 return (cG.rotinas(1, parametro));
	  }
	
	private boolean verificaChaveL() {
		ControleGeral cG = new ControleGeral(0); 
	    String parametro[] = {jTxtCodigo.getText(), "codigo", "Lote"};
		 return (cG.rotinas(1, parametro));
	  }
}
