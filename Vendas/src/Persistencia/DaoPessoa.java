...
public class DaoPessoa implements DaoBasico {
   ...
   @Override
    public boolean incluir(Object o){
      ...
      try{
          ...
          try (PreparedStatement pS = con.prepareStatement(inst)) {
              ...
              pS.setString(5 , p.getTelefone1());  
              pS.setString(6 , p.getTelefone2());  
    ...
    public boolean alterar(Object o){
      boolean result = true;  
      Pessoa p = (Pessoa) o;
      String inst = "Update Pessoa set Nome = ?, Endereco = ?, Definicao = ?, Tele1 = ?,";
             inst += " Tele2 = ?, Email = ? where Identifica = ?";
      try{
          Connection con = DaoConexaoAB.getInstancia().getCon();
          try (PreparedStatement pS = con.prepareStatement(inst)) {
              ...
              pS.setString(3, p.getDefinicao() == 'C' ? "Cliente" : 
                      p.getDefinicao() == 'O' ? "Fornecedor" : "Funcionario");
              pS.setString(4 , p.getTelefone1());  
              pS.setString(5 , p.getTelefone2());
              pS.setString(6, p.getEmail());
              pS.setInt(7, p.getIdentifica());
    ...
    public Object busca(int identifica, int nada) {
        ...
        try {  
            ...
            try (PreparedStatement pS = con.prepareStatement(inst)) {
                ...
                if (rS.next()) {
                   switch(rS.getString("Definicao").charAt(0)) {
                       case 'C' : Cliente c = (Cliente) dC.busca(identifica, 0);
                                          p = c;
                                         p.setDefinicao('C');
                                         break;
                       case 'F' :  Funcionario f = (Funcionario) dF.busca(identifica, 0);    
                                           p = f;
                                           p.setDefinicao('F');
                                           break;
                       default : Fornecedor o = (Fornecedor) dFo.busca(identifica, 0);
                                         p = o;
                                         p.setDefinicao('O');
                   } 
                   ...
                   p.setTelefone1(rS.getString("Tele1"));
                   p.setTelefone2(rS.getString("Tele2"));
